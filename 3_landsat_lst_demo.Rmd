...
Now we can also load vector data for hamburg, in this case a GeoJSON file. What's great about `terra` is that the file type of vector data doesn't really matter - it can handle it all.

```{r}
hamburg = vect('data/hamburg.geojson')
plot(hamburg)
```
...
```{r}
get_relative_lst = function(raster_file, vector_file) {
  
  # load files
  raster = rast(raster_file)
  vector = vect(vector_file)
...
```{r}
relative_lst_list = lapply(landsat_list, get_relative_lst, "data/hamburg.geojson")
```
